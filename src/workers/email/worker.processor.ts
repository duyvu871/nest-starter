...
export class EmailProcessor extends WorkerHost {
    ...
    async process(job: Job<EmailJobData>): Promise<void> {
        const { to, subject, text, html, template, context, idempotencyKey } = job.data;

        // Idempotency check - skip if already processed
        if (idempotencyKey && this.processedJobs.has(idempotencyKey)) {
            this.logger.log(`Skipping duplicate job ${job.id} with idempotency key: ${idempotencyKey}`);
            return;
        }
        ...
        try {
            ...
            if (template && context) {
                ...
                // Send regular email
                await this.emailService.sendEmail(to, subject || 'No Subject', text || '', html);
            }
